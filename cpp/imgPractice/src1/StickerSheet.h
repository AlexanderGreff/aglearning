...
class Coordinates
{
  ...
  public:
    Coordinates(int x = 0, int y = 0) { posX = x; posY = y; }
    ...
    {
        Coordinates res;
        res.posX = this->posX +  obj.posX;
        res.posY = this->posY + obj.posY;
        return res;
    }
    Coordinates min(const Coordinates& a)
    {
      return Coordinates(std::min(a.posX,this->posX),std::min(a.posY,this->posY));
    }

    Coordinates max(const Coordinates& a)
    {
      return Coordinates(std::max(a.posX,this->posX),std::max(a.posY,this->posY));
    }
...
private:
  ...
  std::vector<Coordinates> layerCoords_;
  Image baseImage_;
public:
  /**
